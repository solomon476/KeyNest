...
import { getNotifications, markNotificationRead, getConversation, sendMessage } from '../../services/api';
...
export default function CommunicationHub({ onClose }) {
  ...
  // We'll just show the UI for sending a message to a default recipient
  const [chatPartnerId, setChatPartnerId] = useState(2);
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {messages.map(m => (
                <div key={m.id} style={{ 
                  ...
                  alignSelf: m.sender_id === 2 ? 'flex-start' : 'flex-end',
                  backgroundColor: m.sender_id === 2 ? '#E2E8F0' : 'var(--color-primary)',
                  color: m.sender_id === 2 ? '#1E293B' : '#fff'
                }}>
                  {m.content}
                </div>
              ))}
              {messages.length === 0 && <p style={{ textAlign: 'center', color: '#94A3B8' }}>No messages yet.</p>}
            </div>
